...
_9_main:
    ...
    mov rbp, rsp ; establish new frame base
    sub rsp, 16 ; reserve stack space for locals
    mov [rbp-8], rdi ; store _8_main arg in frame
    ...
    syscall
    push r12 ; preserve current environment
    lea rdi, [rel _10] ; point to string literal
...
_9_main_unwrapper:
    ...
    mov [rbp-8], rdi ; store env_end arg in frame
    mov r12, [rbp-8] ; load operand
    mov rax, [r12-8] ; load _8_main env field
    mov [rbp-16], rax ; store value
    mov rdi, r12 ; use pinned __env_end env_end pointer
    call release_heap_ptr ; release __env_end closure environment
...
section .rodata
_10:
    dq _10_data, 10, 0, 0 ; data, byte length, heap base, heap size
_10_data:
    db "callable!", 10, 0
