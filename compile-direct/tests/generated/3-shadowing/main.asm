...
_348_bar_unwrapper:
    ...
    leave ; unwind before named jump
    jmp _348_bar
global _348_bar_deep_release
_348_bar_deep_release:
    push rbp ; save executor frame pointer
    mov rbp, rsp ; establish new frame base
    sub rsp, 32 ; reserve stack space for locals
    mov [rbp-8], rdi ; store env_end arg in frame
    ...
    mov rbx, 1 ; operand literal
    cmp rax, rbx
    jg _348_bar_release_skip_0
    mov rax, [r12-16] ; load _348_bar_release_field_0 env field
    mov [rbp-24], rax ; store value
    push r12 ; preserve current environment
    mov rdi, [rbp-24] ; load operand
    mov rax, [rdi+8] ; load closure release helper
    call rax ; recursively release closure
    pop r12 ; restore current environment
_348_bar_release_skip_0:
    mov rax, [rbp-16] ; load operand
    mov rbx, 0 ; operand literal
    cmp rax, rbx
    jg _348_bar_release_skip_1
    mov rax, [r12-8] ; load _348_bar_release_field_1 env field
    mov [rbp-32], rax ; store value
    push r12 ; preserve current environment
    mov rdi, [rbp-32] ; load operand
    call release_descriptor_ptr ; release owned descriptor
    pop r12 ; restore current environment
_348_bar_release_skip_1:
    mov rdi, r12 ; use pinned __env_end env_end pointer
    call release_heap_ptr ; release __env_end closure environment
    leave
    ret
...
_348_bar_deepcopy:
    ...
    mov rbx, 1 ; operand literal
    cmp rax, rbx
    jg _348_bar_deepcopy_skip_0
    mov rcx, [r12-16] ; load field pointer
    mov rdi, rcx ; copy pointer argument for deepcopy
    call deepcopy_heap_ptr ; duplicate owned pointer
    mov [r12-16], rax ; store duplicated pointer
    mov [rbp-24], rax ; store value
_348_bar_deepcopy_skip_0:
    mov rax, [rbp-16] ; load operand
    mov rbx, 0 ; operand literal
    cmp rax, rbx
    jg _348_bar_deepcopy_skip_1
    mov rcx, [r12-8] ; load field pointer
    mov rdi, rcx ; copy pointer argument for deepcopy
    call clone_descriptor_ptr ; duplicate owned pointer
    mov [r12-8], rax ; store duplicated pointer
    mov [rbp-32], rax ; store value
_348_bar_deepcopy_skip_1:
    leave
    ret
...
_347_bar_deep_release:
    ...
    mov rbx, 0 ; operand literal
    cmp rax, rbx
    jg _347_bar_release_skip_0
    mov rax, [r12-8] ; load _347_bar_release_field_0 env field
    mov [rbp-24], rax ; store value
    push r12 ; preserve current environment
    mov rdi, [rbp-24] ; load operand
    call release_descriptor_ptr ; release owned descriptor
    pop r12 ; restore current environment
_347_bar_release_skip_0:
    mov rdi, r12 ; use pinned __env_end env_end pointer
    call release_heap_ptr ; release __env_end closure environment
    leave
    ret
...
_347_bar_deepcopy:
    ...
    mov rbx, 0 ; operand literal
    cmp rax, rbx
    jg _347_bar_deepcopy_skip_0
    mov rcx, [r12-8] ; load field pointer
    mov rdi, rcx ; copy pointer argument for deepcopy
    call clone_descriptor_ptr ; duplicate owned pointer
    mov [r12-8], rax ; store duplicated pointer
    mov [rbp-24], rax ; store value
_347_bar_deepcopy_skip_0:
    leave
    ret
...
bar:
    ...
    mov rbx, [rbp-160] ; original closure _331___rgo_7374642f666d74__new to ____comptime_64_arg_clone_1 env_end pointer for clone
    mov rbx, rbx ; clone source env_end pointer
    mov r13, [rbx+24] ; load env size metadata for clone
    mov r14, [rbx+32] ; load heap size metadata for clone
    mov r12, rbx ; compute env base pointer for clone
    ...
    js __rgo_allocation_failed ; mmap failed
    mov rbx, rax ; closure env base pointer
    lea rax, [rel __comptime_63] ; point to string literal
    mov [rbx+0], rax ; capture arg into env
...
_358_foo_bar:
    ...
    leave ; unwind before jumping
    jmp rax
global _358_foo_bar_unwrapper
_358_foo_bar_unwrapper:
    push rbp ; save executor frame pointer
    mov rbp, rsp ; establish new frame base
    sub rsp, 32 ; reserve stack space for locals
    mov [rbp-8], rdi ; store env_end arg in frame
    ...
    leave ; unwind before named jump
    jmp _358_foo_bar
global _358_foo_bar_deep_release
_358_foo_bar_deep_release:
    push rbp ; save executor frame pointer
    mov rbp, rsp ; establish new frame base
    sub rsp, 32 ; reserve stack space for locals
    mov [rbp-8], rdi ; store env_end arg in frame
    ...
    mov rbx, 1 ; operand literal
    cmp rax, rbx
    jg _358_foo_bar_release_skip_0
    mov rax, [r12-16] ; load _358_foo_bar_release_field_0 env field
    mov [rbp-24], rax ; store value
    push r12 ; preserve current environment
    mov rdi, [rbp-24] ; load operand
    mov rax, [rdi+8] ; load closure release helper
    call rax ; recursively release closure
    pop r12 ; restore current environment
_358_foo_bar_release_skip_0:
    mov rax, [rbp-16] ; load operand
    mov rbx, 0 ; operand literal
    cmp rax, rbx
    jg _358_foo_bar_release_skip_1
    mov rax, [r12-8] ; load _358_foo_bar_release_field_1 env field
    mov [rbp-32], rax ; store value
    push r12 ; preserve current environment
    mov rdi, [rbp-32] ; load operand
    call release_descriptor_ptr ; release owned descriptor
    pop r12 ; restore current environment
_358_foo_bar_release_skip_1:
    mov rdi, r12 ; use pinned __env_end env_end pointer
    call release_heap_ptr ; release __env_end closure environment
    leave
    ret

global _358_foo_bar_deepcopy
_358_foo_bar_deepcopy:
    push rbp ; save executor frame pointer
    mov rbp, rsp ; establish new frame base
    sub rsp, 32 ; reserve stack space for locals
    mov [rbp-8], rdi ; store env_end arg in frame
    ...
    mov rbx, 1 ; operand literal
    cmp rax, rbx
    jg _358_foo_bar_deepcopy_skip_0
    mov rcx, [r12-16] ; load field pointer
    mov rdi, rcx ; copy pointer argument for deepcopy
    call deepcopy_heap_ptr ; duplicate owned pointer
    mov [r12-16], rax ; store duplicated pointer
    mov [rbp-24], rax ; store value
_358_foo_bar_deepcopy_skip_0:
    mov rax, [rbp-16] ; load operand
    mov rbx, 0 ; operand literal
    cmp rax, rbx
    jg _358_foo_bar_deepcopy_skip_1
    mov rcx, [r12-8] ; load field pointer
    mov rdi, rcx ; copy pointer argument for deepcopy
    call clone_descriptor_ptr ; duplicate owned pointer
    mov [r12-8], rax ; store duplicated pointer
    mov [rbp-32], rax ; store value
_358_foo_bar_deepcopy_skip_1:
    leave
    ret
...
_357_foo_bar:
    ...
    leave ; unwind before named jump
    jmp _358_foo_bar
global _357_foo_bar_unwrapper
_357_foo_bar_unwrapper:
    push rbp ; save executor frame pointer
    mov rbp, rsp ; establish new frame base
    sub rsp, 32 ; reserve stack space for locals
    mov [rbp-8], rdi ; store env_end arg in frame
    ...
    leave ; unwind before named jump
    jmp _357_foo_bar
global _357_foo_bar_deep_release
_357_foo_bar_deep_release:
    push rbp ; save executor frame pointer
    mov rbp, rsp ; establish new frame base
    sub rsp, 32 ; reserve stack space for locals
    mov [rbp-8], rdi ; store env_end arg in frame
    ...
    mov rbx, 1 ; operand literal
    cmp rax, rbx
    jg _357_foo_bar_release_skip_0
    mov rax, [r12-16] ; load _357_foo_bar_release_field_0 env field
    mov [rbp-24], rax ; store value
    push r12 ; preserve current environment
    mov rdi, [rbp-24] ; load operand
    mov rax, [rdi+8] ; load closure release helper
    call rax ; recursively release closure
    pop r12 ; restore current environment
_357_foo_bar_release_skip_0:
    mov rax, [rbp-16] ; load operand
    mov rbx, 0 ; operand literal
    cmp rax, rbx
    jg _357_foo_bar_release_skip_1
    mov rax, [r12-8] ; load _357_foo_bar_release_field_1 env field
    mov [rbp-32], rax ; store value
    push r12 ; preserve current environment
    mov rdi, [rbp-32] ; load operand
    call release_descriptor_ptr ; release owned descriptor
    pop r12 ; restore current environment
_357_foo_bar_release_skip_1:
    mov rdi, r12 ; use pinned __env_end env_end pointer
    call release_heap_ptr ; release __env_end closure environment
    leave
    ret

global _357_foo_bar_deepcopy
_357_foo_bar_deepcopy:
    push rbp ; save executor frame pointer
    mov rbp, rsp ; establish new frame base
    sub rsp, 32 ; reserve stack space for locals
    mov [rbp-8], rdi ; store env_end arg in frame
    ...
    mov rbx, 1 ; operand literal
    cmp rax, rbx
    jg _357_foo_bar_deepcopy_skip_0
    mov rcx, [r12-16] ; load field pointer
    mov rdi, rcx ; copy pointer argument for deepcopy
    call deepcopy_heap_ptr ; duplicate owned pointer
    mov [r12-16], rax ; store duplicated pointer
    mov [rbp-24], rax ; store value
_357_foo_bar_deepcopy_skip_0:
    mov rax, [rbp-16] ; load operand
    mov rbx, 0 ; operand literal
    cmp rax, rbx
    jg _357_foo_bar_deepcopy_skip_1
    mov rcx, [r12-8] ; load field pointer
    mov rdi, rcx ; copy pointer argument for deepcopy
    call clone_descriptor_ptr ; duplicate owned pointer
    mov [r12-8], rax ; store duplicated pointer
    mov [rbp-32], rax ; store value
_357_foo_bar_deepcopy_skip_1:
    leave
    ret
...
_354_foo_bar:
    ...
    mov rax, r12 ; copy _331___rgo_7374642f666d74__new closure env_end to rax
    mov [rbp-184], rax ; store value
    mov rbx, [rbp-184] ; original closure _331___rgo_7374642f666d74__new to ____comptime_31_arg_clone_1 env_end pointer for clone
    mov rbx, rbx ; clone source env_end pointer
    mov r13, [rbx+24] ; load env size metadata for clone
    mov r14, [rbx+32] ; load heap size metadata for clone
    mov r12, rbx ; compute env base pointer for clone
    ...
    mov rax, 64 ; store heap size metadata
    mov qword [r12+32], rax ; heap size metadata
    lea rax, [_357_foo_bar_unwrapper] ; load unwrapper entry point
    mov qword [r12+0], rax ; store unwrapper entry in metadata
    lea rax, [_357_foo_bar_deep_release] ; load release helper entry point
    mov qword [r12+8], rax ; store release pointer in metadata
    lea rax, [_357_foo_bar_deepcopy] ; load deep copy helper entry point
    mov qword [r12+16], rax ; store deep copy pointer in metadata
    mov qword [r12+40], 1 ; store num_remaining
    ...
    leave
    jmp rax
global _354_foo_bar_unwrapper
_354_foo_bar_unwrapper:
    push rbp ; save executor frame pointer
    mov rbp, rsp ; establish new frame base
    sub rsp, 16 ; reserve stack space for locals
    mov [rbp-8], rdi ; store env_end arg in frame
    ...
    leave ; unwind before named jump
    jmp _354_foo_bar
global _354_foo_bar_deep_release
_354_foo_bar_deep_release:
    push rbp ; save executor frame pointer
    mov rbp, rsp ; establish new frame base
    sub rsp, 32 ; reserve stack space for locals
    mov [rbp-8], rdi ; store env_end arg in frame
    ...
    mov rbx, 0 ; operand literal
    cmp rax, rbx
    jg _354_foo_bar_release_skip_0
    mov rax, [r12-8] ; load _354_foo_bar_release_field_0 env field
    mov [rbp-24], rax ; store value
    push r12 ; preserve current environment
    mov rdi, [rbp-24] ; load operand
    mov rax, [rdi+8] ; load closure release helper
    call rax ; recursively release closure
    pop r12 ; restore current environment
_354_foo_bar_release_skip_0:
    mov rdi, r12 ; use pinned __env_end env_end pointer
    call release_heap_ptr ; release __env_end closure environment
    leave
    ret

global _354_foo_bar_deepcopy
_354_foo_bar_deepcopy:
    push rbp ; save executor frame pointer
    mov rbp, rsp ; establish new frame base
    sub rsp, 32 ; reserve stack space for locals
    mov [rbp-8], rdi ; store env_end arg in frame
    ...
    mov rbx, 0 ; operand literal
    cmp rax, rbx
    jg _354_foo_bar_deepcopy_skip_0
    mov rcx, [r12-8] ; load field pointer
    mov rdi, rcx ; copy pointer argument for deepcopy
    call deepcopy_heap_ptr ; duplicate owned pointer
    mov [r12-8], rax ; store duplicated pointer
    mov [rbp-24], rax ; store value
_354_foo_bar_deepcopy_skip_0:
    leave
    ret
...
section .rodata
__comptime_63:
    dq __comptime_63_data, 14, 0, 0 ; data, byte length, heap base, heap size
__comptime_63_data:
    db "invalid format", 0
__comptime_65:
