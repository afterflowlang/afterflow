...
_34_handler:
    ...
    leave ; unwind before jumping
    jmp rax
global _34_handler_unwrapper
_34_handler_unwrapper:
    push rbp ; save executor frame pointer
    mov rbp, rsp ; establish new frame base
    sub rsp, 32 ; reserve stack space for locals
    mov [rbp-8], rdi ; store env_end arg in frame
    ...
    leave ; unwind before named jump
    jmp _34_handler
global _34_handler_deep_release
_34_handler_deep_release:
    push rbp ; save executor frame pointer
    mov rbp, rsp ; establish new frame base
    sub rsp, 32 ; reserve stack space for locals
    mov [rbp-8], rdi ; store env_end arg in frame
    ...
    mov rbx, 1 ; operand literal
    cmp rax, rbx
    jg _34_handler_release_skip_0
    mov rax, [r12-16] ; load _34_handler_release_field_0 env field
    mov [rbp-24], rax ; store value
    mov rdi, [rbp-24] ; load operand
    call release_heap_ptr ; release heap pointer
_34_handler_release_skip_0:
    mov rdi, r12 ; use pinned __env_end env_end pointer
    call release_heap_ptr ; release __env_end closure environment
    leave
    ret

global _34_handler_deepcopy
_34_handler_deepcopy:
    push rbp ; save executor frame pointer
    mov rbp, rsp ; establish new frame base
    sub rsp, 32 ; reserve stack space for locals
    mov [rbp-8], rdi ; store env_end arg in frame
    ...
    mov rbx, 1 ; operand literal
    cmp rax, rbx
    jg _34_handler_deepcopy_skip_0
    mov rcx, [r12-16] ; load field pointer
    mov rdi, rcx ; copy pointer argument for deepcopy
    call deepcopy_heap_ptr ; duplicate heap pointer
    mov [r12-16], rax ; store duplicated pointer
    mov [rbp-24], rax ; store value
_34_handler_deepcopy_skip_0:
    leave
    ret
...
handler:
    ...
    lea rax, [_34_handler_unwrapper] ; load unwrapper entry point
    mov qword [r12+0], rax ; store unwrapper entry in metadata
    lea rax, [_34_handler_deep_release] ; load release helper entry point
    mov qword [r12+8], rax ; store release pointer in metadata
    lea rax, [_34_handler_deepcopy] ; load deep copy helper entry point
    mov qword [r12+16], rax ; store deep copy pointer in metadata
    mov qword [r12+40], 1 ; store num_remaining
    mov rax, r12 ; copy _35_handler closure env_end to rax
    mov [rbp-24], rax ; store value
    mov rax, [rbp-8] ; load operand
    push rax ; stack arg
...
_40_end_unwrapper:
    ...
    leave ; unwind before named jump
    jmp _40_end
global _40_end_deep_release
_40_end_deep_release:
    push rbp ; save executor frame pointer
    mov rbp, rsp ; establish new frame base
    sub rsp, 16 ; reserve stack space for locals
    mov [rbp-8], rdi ; store env_end arg in frame
...
_38_end:
    ...
    mov qword [r12+32], rax ; heap size metadata
    lea rax, [_40_end_unwrapper] ; load unwrapper entry point
    mov qword [r12+0], rax ; store unwrapper entry in metadata
    lea rax, [_40_end_deep_release] ; load release helper entry point
    mov qword [r12+8], rax ; store release pointer in metadata
    lea rax, [_40_end_deepcopy] ; load deep copy helper entry point
    mov qword [r12+16], rax ; store deep copy pointer in metadata
    mov qword [r12+40], 0 ; store num_remaining
    mov rax, r12 ; copy _40_end closure env_end to rax
    mov [rbp-16], rax ; store value
    mov rax, [rbp-8] ; load operand
    push rax ; stack arg
    pop rdi ; restore arg into register
...
_38_end_unwrapper:
    ...
    leave ; unwind before named jump
    jmp _38_end
global _38_end_deep_release
_38_end_deep_release:
    push rbp ; save executor frame pointer
    mov rbp, rsp ; establish new frame base
    sub rsp, 16 ; reserve stack space for locals
    mov [rbp-8], rdi ; store env_end arg in frame
...
global end
    ...
    mov qword [r12+32], rax ; heap size metadata
    lea rax, [_38_end_unwrapper] ; load unwrapper entry point
    mov qword [r12+0], rax ; store unwrapper entry in metadata
    lea rax, [_38_end_deep_release] ; load release helper entry point
    mov qword [r12+8], rax ; store release pointer in metadata
    lea rax, [_38_end_deepcopy] ; load deep copy helper entry point
    mov qword [r12+16], rax ; store deep copy pointer in metadata
    mov qword [r12+40], 1 ; store num_remaining
    mov rax, r12 ; copy _38_end closure env_end to rax
...
global _30_iterate_iterate_inner
_30_iterate_iterate_inner:
    push rbp ; save executor frame pointer
    mov rbp, rsp ; establish new frame base
    ...
    leave ; unwind before jumping
    jmp rax ; tail call into closure
global _30_iterate_iterate_inner_unwrapper
_30_iterate_iterate_inner_unwrapper:
    push rbp ; save executor frame pointer
    mov rbp, rsp ; establish new frame base
    sub rsp, 48 ; reserve stack space for locals
    mov [rbp-8], rdi ; store env_end arg in frame
    ...
    leave ; unwind before named jump
    jmp _30_iterate_iterate_inner
global _30_iterate_iterate_inner_deep_release
_30_iterate_iterate_inner_deep_release:
    push rbp ; save executor frame pointer
    mov rbp, rsp ; establish new frame base
    sub rsp, 48 ; reserve stack space for locals
    mov [rbp-8], rdi ; store env_end arg in frame
    ...
    cmp rax, rbx
    jg _30_iterate_iterate_inner_release_skip_0
    mov rax, [r12-40] ; load _30_iterate_iterate_inner_release_field_0 env field
    mov [rbp-24], rax ; store value
    mov rdi, [rbp-24] ; load operand
    call release_heap_ptr ; release heap pointer
_30_iterate_iterate_inner_release_skip_0:
    mov rax, [rbp-16] ; load operand
    mov rbx, 3 ; operand literal
    cmp rax, rbx
    jg _30_iterate_iterate_inner_release_skip_1
    mov rax, [r12-32] ; load _30_iterate_iterate_inner_release_field_1 env field
    mov [rbp-32], rax ; store value
    mov rdi, [rbp-32] ; load operand
    call release_heap_ptr ; release heap pointer
_30_iterate_iterate_inner_release_skip_1:
    mov rax, [rbp-16] ; load operand
    mov rbx, 2 ; operand literal
    cmp rax, rbx
    jg _30_iterate_iterate_inner_release_skip_2
    mov rax, [r12-24] ; load _30_iterate_iterate_inner_release_field_2 env field
    mov [rbp-40], rax ; store value
    mov rdi, [rbp-40] ; load operand
    call release_heap_ptr ; release heap pointer
_30_iterate_iterate_inner_release_skip_2:
    mov rax, [rbp-16] ; load operand
    mov rbx, 0 ; operand literal
    cmp rax, rbx
    jg _30_iterate_iterate_inner_release_skip_4
    mov rax, [r12-8] ; load _30_iterate_iterate_inner_release_field_4 env field
    mov [rbp-48], rax ; store value
    mov rdi, [rbp-48] ; load operand
    call release_heap_ptr ; release heap pointer
_30_iterate_iterate_inner_release_skip_4:
    mov rdi, r12 ; use pinned __env_end env_end pointer
    call release_heap_ptr ; release __env_end closure environment
    leave
    ret

global _30_iterate_iterate_inner_deepcopy
_30_iterate_iterate_inner_deepcopy:
    push rbp ; save executor frame pointer
    mov rbp, rsp ; establish new frame base
    sub rsp, 48 ; reserve stack space for locals
    mov [rbp-8], rdi ; store env_end arg in frame
    ...
    mov rbx, 4 ; operand literal
    cmp rax, rbx
    jg _30_iterate_iterate_inner_deepcopy_skip_0
    mov rcx, [r12-40] ; load field pointer
    mov rdi, rcx ; copy pointer argument for deepcopy
    call deepcopy_heap_ptr ; duplicate heap pointer
    mov [r12-40], rax ; store duplicated pointer
    mov [rbp-24], rax ; store value
_30_iterate_iterate_inner_deepcopy_skip_0:
    mov rax, [rbp-16] ; load operand
    mov rbx, 3 ; operand literal
    cmp rax, rbx
    jg _30_iterate_iterate_inner_deepcopy_skip_1
    mov rcx, [r12-32] ; load field pointer
    mov rdi, rcx ; copy pointer argument for deepcopy
    call deepcopy_heap_ptr ; duplicate heap pointer
    mov [r12-32], rax ; store duplicated pointer
    mov [rbp-32], rax ; store value
_30_iterate_iterate_inner_deepcopy_skip_1:
    mov rax, [rbp-16] ; load operand
    mov rbx, 2 ; operand literal
    cmp rax, rbx
    jg _30_iterate_iterate_inner_deepcopy_skip_2
    mov rcx, [r12-24] ; load field pointer
    mov rdi, rcx ; copy pointer argument for deepcopy
    call deepcopy_heap_ptr ; duplicate heap pointer
    mov [r12-24], rax ; store duplicated pointer
    mov [rbp-40], rax ; store value
_30_iterate_iterate_inner_deepcopy_skip_2:
    mov rax, [rbp-16] ; load operand
    mov rbx, 0 ; operand literal
    cmp rax, rbx
    jg _30_iterate_iterate_inner_deepcopy_skip_4
    mov rcx, [r12-8] ; load field pointer
    mov rdi, rcx ; copy pointer argument for deepcopy
    call deepcopy_heap_ptr ; duplicate heap pointer
    mov [r12-8], rax ; store duplicated pointer
    mov [rbp-48], rax ; store value
_30_iterate_iterate_inner_deepcopy_skip_4:
    leave
    ret
...
iterate:
    ...
    mov qword [r12+32], rax ; heap size metadata
    lea rax, [_30_iterate_iterate_inner_unwrapper] ; load unwrapper entry point
    mov qword [r12+0], rax ; store unwrapper entry in metadata
    lea rax, [_30_iterate_iterate_inner_deep_release] ; load release helper entry point
    mov qword [r12+8], rax ; store release pointer in metadata
    lea rax, [_30_iterate_iterate_inner_deepcopy] ; load deep copy helper entry point
    mov qword [r12+16], rax ; store deep copy pointer in metadata
    mov qword [r12+40], 2 ; store num_remaining
    mov rax, r12 ; copy _31_iterate_inner closure env_end to rax
    mov [rbp-40], rax ; store value
...
section .rodata
_32:
    dq _32_data, 4 ; string data pointer and byte length
_32_data:
    db "%d, ", 0
_36:
    dq _36_data, 4 ; string data pointer and byte length
_36_data:
    db "end", 10, 0
