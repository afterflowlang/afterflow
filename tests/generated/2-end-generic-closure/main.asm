...
main:
    ...
    lea rax, [unused_unwrapper] ; load unwrapper entry point
    mov qword [r12+0], rax ; store unwrapper entry in metadata
    lea rax, [unused_deep_release] ; load release helper entry point
    mov qword [r12+8], rax ; store release pointer in metadata
    lea rax, [unused_deepcopy] ; load deep copy helper entry point
    mov qword [r12+16], rax ; store deep copy pointer in metadata
    mov qword [r12+40], 0 ; store num_remaining
    mov rax, r12 ; copy unused closure env_end to rax
    ...
    mov qword [r12+16], rax ; store deep copy pointer in metadata
    mov qword [r12+40], 0 ; store num_remaining
    mov rax, r12 ; copy _14___rgo_737464__end closure env_end to rax
    mov [rbp-24], rax ; store value
    mov rax, [rbp-24] ; load operand
    push rax ; stack arg
...
section .rodata
_9:
    dq _9_data, 0, 0, 0 ; data, byte length, heap base, heap size
_9_data:
    db "", 0
