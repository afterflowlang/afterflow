...
main:
    ...
    mov rbx, [rbp-144] ; original closure _331___rgo_7374642f666d74__new to ____comptime_26_arg_clone_1 env_end pointer for clone
    mov rbx, rbx ; clone source env_end pointer
    mov r13, [rbx+24] ; load env size metadata for clone
    mov r14, [rbx+32] ; load heap size metadata for clone
    mov r12, rbx ; compute env base pointer for clone
    ...
    js __rgo_allocation_failed ; mmap failed
    mov rbx, rax ; closure env base pointer
    lea rax, [rel __comptime_25] ; point to string literal
    mov [rbx+0], rax ; capture arg into env
    ...
    mov rbx, [rbp-144] ; original closure _331___rgo_7374642f666d74__new to ____comptime_30_arg_clone_1 env_end pointer for clone
    mov rbx, rbx ; clone source env_end pointer
    mov r13, [rbx+24] ; load env size metadata for clone
    mov r14, [rbx+32] ; load heap size metadata for clone
    mov r12, rbx ; compute env base pointer for clone
...
section .rodata
__comptime_25:
    dq __comptime_25_data, 14, 0, 0 ; data, byte length, heap base, heap size
__comptime_25_data:
    db "invalid format", 0
__comptime_29:
