...
_10_print_unwrapper:
    ...
    jmp _10_print
global _10_print_deep_release
_10_print_deep_release:
    push rbp ; save executor frame pointer
    mov rbp, rsp ; establish new frame base
    sub rsp, 32 ; reserve stack space for locals
    mov [rbp-8], rdi ; store env_end arg in frame
    ...
    cmp rax, rbx
    jg _10_print_release_skip_0
    mov rax, [r12-16] ; load _10_print_release_field_0 env field
    mov [rbp-24], rax ; store value
    mov rdi, [rbp-24] ; load operand
    call release_heap_ptr ; release heap pointer
_10_print_release_skip_0:
    mov rdi, r12 ; use pinned __env_end env_end pointer
    call release_heap_ptr ; release __env_end closure environment
    leave
    ret
...
print:
    ...
    lea rax, [_10_print_unwrapper] ; load unwrapper entry point
    mov qword [r12+0], rax ; store unwrapper entry in metadata
    lea rax, [_10_print_deep_release] ; load release helper entry point
    mov qword [r12+8], rax ; store release pointer in metadata
    lea rax, [_10_print_deepcopy] ; load deep copy helper entry point
    mov qword [r12+16], rax ; store deep copy pointer in metadata
    mov qword [r12+40], 1 ; store num_remaining
    mov rax, r12 ; copy _11_print closure env_end to rax
    mov [rbp-24], rax ; store value
...
section .rodata
_8:
    dq _8_data, 25 ; string data pointer and byte length
_8_data:
    db "The winning number is %s", 10, 0
_7:
    dq _7_data, 2 ; string data pointer and byte length
_7_data:
    db "42", 0
