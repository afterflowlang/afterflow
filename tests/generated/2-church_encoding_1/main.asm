...
_356_main:
    ...
    mov rbx, [rbp-152] ; original closure _359_main to ____comptime_24_arg_clone_1 env_end pointer for clone
    mov rbx, rbx ; clone source env_end pointer
    mov r13, [rbx+24] ; load env size metadata for clone
    mov r14, [rbx+32] ; load heap size metadata for clone
    mov r12, rbx ; compute env base pointer for clone
    ...
    mov rbx, [rbp-152] ; original closure _359_main to ____comptime_26_arg_clone_0 env_end pointer for clone
    mov rbx, rbx ; clone source env_end pointer
    mov r13, [rbx+24] ; load env size metadata for clone
    mov r14, [rbx+32] ; load heap size metadata for clone
    mov r12, rbx ; compute env base pointer for clone
    ...
    syscall ; allocate env pages
    test rax, rax
    js _356_main_bytes_build_allocation_failed_0
    mov rbx, rax
    ...
    leave
    jmp rax
_356_main_bytes_build_allocation_failed_0:
    push r12 ; preserve current environment
