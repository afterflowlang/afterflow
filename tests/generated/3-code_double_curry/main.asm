...
_362_bar_unwrapper:
    ...
    leave ; unwind before named jump
    jmp _362_bar
global _362_bar_deep_release
_362_bar_deep_release:
    push rbp ; save executor frame pointer
    mov rbp, rsp ; establish new frame base
    sub rsp, 16 ; reserve stack space for locals
    mov [rbp-8], rdi ; store env_end arg in frame
...
_360_bar_deep_release:
    ...
    mov rax, [r12-16] ; load _360_bar_release_field_0 env field
    mov [rbp-24], rax ; store value
    push r12 ; preserve current environment
    mov rdi, [rbp-24] ; load operand
    mov rax, [rdi+8] ; load closure release helper
...
_360_bar_release_skip_0:
    ...
    mov rax, [r12-8] ; load _360_bar_release_field_1 env field
    mov [rbp-32], rax ; store value
    push r12 ; preserve current environment
    mov rdi, [rbp-32] ; load operand
    call release_descriptor_ptr ; release owned descriptor
...
_359_bar:
    ...
    mov qword [r12+32], rax ; heap size metadata
    lea rax, [_362_bar_unwrapper] ; load unwrapper entry point
    mov qword [r12+0], rax ; store unwrapper entry in metadata
    lea rax, [_362_bar_deep_release] ; load release helper entry point
    mov qword [r12+8], rax ; store release pointer in metadata
    lea rax, [_362_bar_deepcopy] ; load deep copy helper entry point
    mov qword [r12+16], rax ; store deep copy pointer in metadata
    mov qword [r12+40], 0 ; store num_remaining
    mov rax, r12 ; copy _362_bar closure env_end to rax
    mov [rbp-16], rax ; store value
    mov rax, [rbp-8] ; load operand
    push rax ; stack arg
    mov rax, [rbp-16] ; load operand
...
_359_bar_deep_release:
    ...
    mov rax, [r12-8] ; load _359_bar_release_field_0 env field
    mov [rbp-24], rax ; store value
    push r12 ; preserve current environment
    mov rdi, [rbp-24] ; load operand
    call release_descriptor_ptr ; release owned descriptor
...
bar:
    push rbp ; save executor frame pointer
    mov rbp, rsp ; establish new frame base
    sub rsp, 352 ; reserve stack space for locals
    ...
    mov rbx, [rbp-312] ; original closure _334___rgo_7374642f666d74__new to ____comptime_49_arg_clone_1 env_end pointer for clone
    mov rbx, rbx ; clone source env_end pointer
    mov r13, [rbx+24] ; load env size metadata for clone
    mov r14, [rbx+32] ; load heap size metadata for clone
    mov r12, rbx ; compute env base pointer for clone
    ...
    js __rgo_allocation_failed ; mmap failed
    mov rbx, rax ; closure env base pointer
    lea rax, [rel __comptime_48] ; point to string literal
    mov [rbx+0], rax ; capture arg into env
...
baz:
    ...
    lea rcx, [rel _352] ; point to string literal
    mov [r12-16], rcx ; store env field
...
main:
    ...
    mov qword [r12+16], rax ; store deep copy pointer in metadata
    mov qword [r12+40], 2 ; store num_remaining
    mov rax, r12 ; copy _367_bar closure env_end to rax
...
section .rodata
__comptime_48:
    dq __comptime_48_data, 14, 0, 0 ; data, byte length, heap base, heap size
__comptime_48_data:
    db "invalid format", 0
__comptime_50:
...
__comptime_50_data:
    db "invalid format", 0
_349:
    dq _349_data, 7, 0, 0 ; data, byte length, heap base, heap size
_349_data:
    db "charlie", 0
_352:
    dq _352_data, 3, 0, 0 ; data, byte length, heap base, heap size
_352_data:
    db "bob", 0
_366:
