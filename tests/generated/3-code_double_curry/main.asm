...
_16_bar_unwrapper:
    ...
    jmp _16_bar
global _16_bar_deep_release
_16_bar_deep_release:
    push rbp ; save executor frame pointer
    mov rbp, rsp ; establish new frame base
    sub rsp, 16 ; reserve stack space for locals
    mov [rbp-8], rdi ; store env_end arg in frame
...
_14_bar:
    ...
    lea rax, [_16_bar_unwrapper] ; load unwrapper entry point
    mov qword [r12+0], rax ; store unwrapper entry in metadata
    lea rax, [_16_bar_deep_release] ; load release helper entry point
    mov qword [r12+8], rax ; store release pointer in metadata
    lea rax, [_16_bar_deepcopy] ; load deep copy helper entry point
    mov qword [r12+16], rax ; store deep copy pointer in metadata
    mov qword [r12+40], 0 ; store num_remaining
    mov rax, r12 ; copy _16_bar closure env_end to rax
...
baz:
    ...
    sub rsp, 16 ; reserve stack space for locals
    mov [rbp-8], rdi ; store ok arg in frame
    mov rbx, [rbp-8] ; original closure ok to _11_ok env_end pointer for clone
    ...
    lea rcx, [rel _10] ; point to string literal
...
main:
    ...
    mov rax, r12 ; copy _21_bar closure env_end to rax
...
section .rodata
_12:
    dq _12_data, 20 ; string data pointer and byte length
_12_data:
    db "hello %s, %s and %s", 10, 0
_7:
    dq _7_data, 7 ; string data pointer and byte length
_7_data:
    db "charlie", 0
_10:
    dq _10_data, 3 ; string data pointer and byte length
_10_data:
    db "bob", 0
_20:
    dq _20_data, 5 ; string data pointer and byte length
_20_data:
    db "alice", 0
