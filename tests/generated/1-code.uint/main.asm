...
_5_print_uint:
    ...
    jmp rax
global _5_print_uint_unwrapper
_5_print_uint_unwrapper:
    push rbp ; save executor frame pointer
    mov rbp, rsp ; establish new frame base
    ...
    leave ; unwind before named jump
    jmp _5_print_uint
global _5_print_uint_deep_release
_5_print_uint_deep_release:
    push rbp ; save executor frame pointer
    mov rbp, rsp ; establish new frame base
    sub rsp, 32 ; reserve stack space for locals
    mov [rbp-8], rdi ; store env_end arg in frame
    ...
    cmp rax, rbx
    jg _5_print_uint_release_skip_0
    mov rax, [r12-16] ; load _5_print_uint_release_field_0 env field
    mov [rbp-24], rax ; store value
    mov rdi, [rbp-24] ; load operand
    call release_heap_ptr ; release heap pointer
_5_print_uint_release_skip_0:
    mov rdi, r12 ; use pinned __env_end env_end pointer
    call release_heap_ptr ; release __env_end closure environment
    leave
    ret
...
internal_memcpy_done:
    pop rbp
    ret
global _5_print_uint_deepcopy
_5_print_uint_deepcopy:
    push rbp ; save executor frame pointer
    mov rbp, rsp ; establish new frame base
    sub rsp, 32 ; reserve stack space for locals
    mov [rbp-8], rdi ; store env_end arg in frame
    ...
    mov rbx, 1 ; operand literal
    cmp rax, rbx
    jg _5_print_uint_deepcopy_skip_0
...
print_uint:
    ...
    lea rax, [_5_print_uint_unwrapper] ; load unwrapper entry point
    mov qword [r12+0], rax ; store unwrapper entry in metadata
    lea rax, [_5_print_uint_deep_release] ; load release helper entry point
    mov qword [r12+8], rax ; store release pointer in metadata
    lea rax, [_5_print_uint_deepcopy] ; load deep copy helper entry point
    mov qword [r12+16], rax ; store deep copy pointer in metadata
    mov qword [r12+40], 1 ; store num_remaining
    mov rax, r12 ; copy _6_print_uint closure env_end to rax
    mov [rbp-24], rax ; store value
    ...
    leave ; unwind before jumping
    jmp rax
global print_uint_unwrapper
print_uint_unwrapper:
    push rbp ; save executor frame pointer
    mov rbp, rsp ; establish new frame base
    sub rsp, 32 ; reserve stack space for locals
    mov [rbp-8], rdi ; store env_end arg in frame
    ...
    leave ; unwind before named jump
    jmp print_uint
global print_uint_deep_release
print_uint_deep_release:
    push rbp ; save executor frame pointer
    mov rbp, rsp ; establish new frame base
    sub rsp, 32 ; reserve stack space for locals
    mov [rbp-8], rdi ; store env_end arg in frame
    ...
    cmp rax, rbx
    jg print_uint_release_skip_1
    mov rax, [r12-8] ; load print_uint_release_field_1 env field
    mov [rbp-24], rax ; store value
    mov rdi, [rbp-24] ; load operand
    call release_heap_ptr ; release heap pointer
print_uint_release_skip_1:
    mov rdi, r12 ; use pinned __env_end env_end pointer
    call release_heap_ptr ; release __env_end closure environment
    leave
    ret

global print_uint_deepcopy
print_uint_deepcopy:
    push rbp ; save executor frame pointer
    mov rbp, rsp ; establish new frame base
    sub rsp, 32 ; reserve stack space for locals
