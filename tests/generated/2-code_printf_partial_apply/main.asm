...
_357_main_deep_release:
    ...
    mov rax, [r12-16] ; load _357_main_release_field_0 env field
    mov [rbp-24], rax ; store value
    push r12 ; preserve current environment
    mov rdi, [rbp-24] ; load operand
    mov rax, [rdi+8] ; load closure release helper
...
_347_main:
    ...
    mov rbx, [rbp-152] ; original closure _350_main to ____comptime_25_arg_clone_1 env_end pointer for clone
    mov rbx, rbx ; clone source env_end pointer
    mov r13, [rbx+24] ; load env size metadata for clone
    mov r14, [rbx+32] ; load heap size metadata for clone
    mov r12, rbx ; compute env base pointer for clone
    ...
    mov rbx, [rbp-152] ; original closure _350_main to ____comptime_28_arg_clone_0 env_end pointer for clone
    mov rbx, rbx ; clone source env_end pointer
    mov r13, [rbx+24] ; load env size metadata for clone
    mov r14, [rbx+32] ; load heap size metadata for clone
    mov r12, rbx ; compute env base pointer for clone
    ...
    syscall ; allocate env pages
    test rax, rax
    js _347_main_bytes_build_allocation_failed_0
    mov rbx, rax
    ...
    leave
    jmp rax
_347_main_bytes_build_allocation_failed_0:
    push r12 ; preserve current environment
