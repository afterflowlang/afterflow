...
_6_finish_unwrapper:
    ...
    jmp _6_finish
global _6_finish_deep_release
_6_finish_deep_release:
    push rbp ; save executor frame pointer
    mov rbp, rsp ; establish new frame base
    sub rsp, 16 ; reserve stack space for locals
    mov [rbp-8], rdi ; store env_end arg in frame
...
_4_finish:
    ...
    lea rax, [_6_finish_unwrapper] ; load unwrapper entry point
    mov qword [r12+0], rax ; store unwrapper entry in metadata
    lea rax, [_6_finish_deep_release] ; load release helper entry point
    mov qword [r12+8], rax ; store release pointer in metadata
    lea rax, [_6_finish_deepcopy] ; load deep copy helper entry point
    mov qword [r12+16], rax ; store deep copy pointer in metadata
    mov qword [r12+40], 0 ; store num_remaining
    mov rax, r12 ; copy _6_finish closure env_end to rax
...
_4_finish_unwrapper:
    ...
    mov [rbp-8], rdi ; store env_end arg in frame
    mov r12, [rbp-8] ; load operand
    mov rax, [r12-8] ; load message env field
    mov [rbp-16], rax ; store value
    mov rdi, r12 ; use pinned __env_end env_end pointer
    call release_heap_ptr ; release __env_end closure environment
    ...
    leave ; unwind before named jump
    jmp _4_finish
global _4_finish_deep_release
_4_finish_deep_release:
    push rbp ; save executor frame pointer
    mov rbp, rsp ; establish new frame base
    sub rsp, 16 ; reserve stack space for locals
    mov [rbp-8], rdi ; store env_end arg in frame
...
finish:
    ...
    mov qword [r12+32], rax ; heap size metadata
    lea rax, [_4_finish_unwrapper] ; load unwrapper entry point
    mov qword [r12+0], rax ; store unwrapper entry in metadata
    lea rax, [_4_finish_deep_release] ; load release helper entry point
    mov qword [r12+8], rax ; store release pointer in metadata
    lea rax, [_4_finish_deepcopy] ; load deep copy helper entry point
    mov qword [r12+16], rax ; store deep copy pointer in metadata
    mov qword [r12+40], 1 ; store num_remaining
    mov rax, r12 ; copy _4_finish closure env_end to rax
...
_12_main:
    ...
    mov qword [r12+16], rax ; store deep copy pointer in metadata
    mov qword [r12+40], 1 ; store num_remaining
    mov rax, r12 ; copy _14_finish closure env_end to rax
...
section .rodata
_2:
    dq _2_data, 6 ; string data pointer and byte length
_2_data:
    db "%s %d", 10, 0
prefix:
    dq prefix_data, 5 ; string data pointer and byte length
prefix_data:
    db "value", 0
_10:
    dq _10_data, 6 ; string data pointer and byte length
_10_data:
    db "start ", 0
