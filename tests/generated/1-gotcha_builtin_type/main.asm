...
_5_w_deep_release:
    ...
    cmp rax, rbx
    jg _5_w_release_skip_0
    mov rax, [r12-16] ; load _5_w_release_field_0 env field
    mov [rbp-24], rax ; store value
...
w:
    ...
    lea rax, [_5_w_unwrapper] ; load unwrapper entry point
    mov qword [r12+0], rax ; store unwrapper entry in metadata
    lea rax, [_5_w_deep_release] ; load release helper entry point
    mov qword [r12+8], rax ; store release pointer in metadata
    lea rax, [_5_w_deepcopy] ; load deep copy helper entry point
    mov qword [r12+16], rax ; store deep copy pointer in metadata
    mov qword [r12+40], 1 ; store num_remaining
    mov rax, r12 ; copy _6_w closure env_end to rax
    mov [rbp-24], rax ; store value
...
_3_data:
    db "Gotcha: %s", 0
_7:
    dq _7_data, 3 ; string data pointer and byte length
_7_data:
    db "one", 0
