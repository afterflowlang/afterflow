...
_354_main_deep_release:
    ...
    mov rbx, 1 ; operand literal
    cmp rax, rbx
    jg _354_main_release_skip_0
    mov rax, [r12-16] ; load _354_main_release_field_0 env field
    mov [rbp-24], rax ; store value
    push r12 ; preserve current environment
    mov rdi, [rbp-24] ; load operand
    mov rax, [rdi+8] ; load closure release helper
...
_354_main_deepcopy:
    ...
    mov rbx, 1 ; operand literal
    cmp rax, rbx
    jg _354_main_deepcopy_skip_0
    mov rcx, [r12-16] ; load field pointer
    mov rdi, rcx ; copy pointer argument for deepcopy
    call deepcopy_heap_ptr ; duplicate owned pointer
    mov [r12-16], rax ; store duplicated pointer
    mov [rbp-24], rax ; store value
_354_main_deepcopy_skip_0:
    mov rax, [rbp-16] ; load operand
    mov rbx, 0 ; operand literal
    cmp rax, rbx
...
_353_main_deep_release:
    ...
    mov rax, [r12-8] ; load _353_main_release_field_0 env field
    mov [rbp-24], rax ; store value
    push r12 ; preserve current environment
    mov rdi, [rbp-24] ; load operand
    call release_descriptor_ptr ; release owned descriptor
...
_350_main:
    ...
    mov rbx, [rbp-96] ; original closure _333___rgo_7374642f666d74__new to ____comptime_16_arg_clone_1 env_end pointer for clone
    mov rbx, rbx ; clone source env_end pointer
    mov r13, [rbx+24] ; load env size metadata for clone
    mov r14, [rbx+32] ; load heap size metadata for clone
    mov r12, rbx ; compute env base pointer for clone
...
_362_main:
    ...
    mov rbx, [rbp-208] ; original closure _333___rgo_7374642f666d74__new to ____comptime_57_arg_clone_1 env_end pointer for clone
    mov rbx, rbx ; clone source env_end pointer
    mov r13, [rbx+24] ; load env size metadata for clone
    mov r14, [rbx+32] ; load heap size metadata for clone
    mov r12, rbx ; compute env base pointer for clone
    ...
    js __rgo_allocation_failed ; mmap failed
    mov rbx, rax ; closure env base pointer
    lea rax, [rel __comptime_56] ; point to string literal
    mov [rbx+0], rax ; capture arg into env
...
__comptime_17_data:
    db "invalid format", 0
__comptime_56:
    dq __comptime_56_data, 14, 0, 0 ; data, byte length, heap base, heap size
__comptime_56_data:
    db "invalid format", 0
__comptime_58:
