...
_345_main:
    ...
    mov rbx, [rbp-96] ; original closure _348_main to ____comptime_15_arg_clone_0 env_end pointer for clone
    mov rbx, rbx ; clone source env_end pointer
    mov r13, [rbx+24] ; load env size metadata for clone
    mov r14, [rbx+32] ; load heap size metadata for clone
    mov r12, rbx ; compute env base pointer for clone
    ...
    syscall ; allocate env pages
    test rax, rax
    js _345_main_bytes_build_allocation_failed_0
    mov rbx, rax
    ...
    leave
    jmp rax
_345_main_bytes_build_allocation_failed_0:
    push r12 ; preserve current environment
    ...
    call rax ; recursively release closure
    pop r12 ; restore current environment
    mov r12, [rbp-96] ; load continuation env_end pointer
    mov rax, [r12+0] ; load continuation entry point
    mov rdi, r12 ; pass env_end pointer to continuation
    leave ; unwind before jumping
    jmp rax
...
_361_main:
    ...
    mov rbx, [rbp-208] ; original closure _364_main to ____comptime_52_arg_clone_0 env_end pointer for clone
    mov rbx, rbx ; clone source env_end pointer
    mov r13, [rbx+24] ; load env size metadata for clone
    mov r14, [rbx+32] ; load heap size metadata for clone
    mov r12, rbx ; compute env base pointer for clone
    ...
    syscall ; allocate env pages
    test rax, rax
    js _361_main_bytes_build_allocation_failed_0
    mov rbx, rax
    ...
    leave
    jmp rax
_361_main_bytes_build_allocation_failed_0:
    push r12 ; preserve current environment
