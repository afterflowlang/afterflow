...
_7_foo:
    ...
    mov [rbp-16], rsi ; store done arg in frame
    ...
    mov rax, r12 ; copy _10_baz closure env_end to rax
    ...
    syscall
    push r12 ; preserve current environment
    lea rdi, [rel _8] ; point to string literal
    call release_descriptor_ptr ; release owned descriptor
    pop r12 ; restore current environment
...
_7_foo_unwrapper:
    ...
    mov [rbp-8], rdi ; store env_end arg in frame
    mov r12, [rbp-8] ; load operand
    mov rax, [r12-24] ; load foo env field
    mov [rbp-16], rax ; store value
    mov rax, [r12-16] ; load done env field
    mov [rbp-24], rax ; store value
...
_7_foo_deep_release:
    ...
    cmp rax, rbx
    jg _7_foo_release_skip_0
    mov rax, [r12-24] ; load _7_foo_release_field_0 env field
    mov [rbp-24], rax ; store value
    push r12 ; preserve current environment
    mov rdi, [rbp-24] ; load operand
    mov rax, [rdi+8] ; load closure release helper
    call rax ; recursively release closure
    pop r12 ; restore current environment
_7_foo_release_skip_0:
    mov rax, [rbp-16] ; load operand
    mov rbx, 1 ; operand literal
    cmp rax, rbx
    jg _7_foo_release_skip_1
    mov rax, [r12-16] ; load _7_foo_release_field_1 env field
    mov [rbp-32], rax ; store value
    push r12 ; preserve current environment
...
_4_foo:
    ...
    mov rbp, rsp ; establish new frame base
    sub rsp, 32 ; reserve stack space for locals
    mov [rbp-8], rdi ; store remaining arg in frame
    mov [rbp-16], rsi ; store foo arg in frame
    mov [rbp-24], rdx ; store done arg in frame
...
_4_foo_unwrapper:
    ...
    mov [rbp-8], rdi ; store env_end arg in frame
    mov r12, [rbp-8] ; load operand
    mov rax, [r12-24] ; load remaining env field
    mov [rbp-16], rax ; store value
    mov rax, [r12-16] ; load foo env field
    mov [rbp-24], rax ; store value
    mov rax, [r12-8] ; load done env field
    mov [rbp-32], rax ; store value
    mov rdi, r12 ; use pinned __env_end env_end pointer
    call release_heap_ptr ; release __env_end closure environment
    mov rax, [rbp-32] ; load operand
...
_4_foo_deep_release:
    ...
    mov rbx, 1 ; operand literal
    cmp rax, rbx
    jg _4_foo_release_skip_1
    mov rax, [r12-16] ; load _4_foo_release_field_1 env field
    mov [rbp-24], rax ; store value
    push r12 ; preserve current environment
    mov rdi, [rbp-24] ; load operand
    mov rax, [rdi+8] ; load closure release helper
    call rax ; recursively release closure
    pop r12 ; restore current environment
_4_foo_release_skip_1:
    mov rax, [rbp-16] ; load operand
    mov rbx, 0 ; operand literal
    cmp rax, rbx
    jg _4_foo_release_skip_2
    mov rax, [r12-8] ; load _4_foo_release_field_2 env field
    mov [rbp-32], rax ; store value
    push r12 ; preserve current environment
    mov rdi, [rbp-32] ; load operand
    mov rax, [rdi+8] ; load closure release helper
...
_4_foo_deepcopy:
    ...
    mov rbx, 1 ; operand literal
    cmp rax, rbx
    jg _4_foo_deepcopy_skip_1
    mov rcx, [r12-16] ; load field pointer
    mov rdi, rcx ; copy pointer argument for deepcopy
    call deepcopy_heap_ptr ; duplicate owned pointer
    mov [r12-16], rax ; store duplicated pointer
    mov [rbp-24], rax ; store value
_4_foo_deepcopy_skip_1:
    mov rax, [rbp-16] ; load operand
    mov rbx, 0 ; operand literal
    cmp rax, rbx
    jg _4_foo_deepcopy_skip_2
    mov rcx, [r12-8] ; load field pointer
    mov rdi, rcx ; copy pointer argument for deepcopy
    call deepcopy_heap_ptr ; duplicate owned pointer
    mov [r12-8], rax ; store duplicated pointer
    mov [rbp-32], rax ; store value
_4_foo_deepcopy_skip_2:
    leave
    ret
...
foo:
    ...
    mov [rbp-24], rax ; store value
    mov rbx, [rbp-16] ; original closure done to ___13_foo_arg_clone_2 env_end pointer for clone
    ...
    mov rbx, 0 ; operand literal
    cmp rax, rbx
    je eq_int_done_true_0_0
eq_int__13_foo_false_0_0:
    push r12 ; preserve current environment
    mov rdi, [rbp-16] ; load operand
    mov rax, [rdi+8] ; load closure release helper
    call rax ; recursively release closure
    pop r12 ; restore current environment
    mov rbx, [rbp-40] ; load _13_foo closure env_end pointer
...
eq_int_done_true_0_0:
    push r12 ; preserve current environment
    mov rdi, [rbp-40] ; load operand
    mov rax, [rdi+8] ; load closure release helper
    call rax ; recursively release closure
    pop r12 ; restore current environment
    mov rbx, [rbp-16] ; load done closure env_end pointer
    mov rdi, rbx ; pass env_end pointer to closure
    mov rax, [rdi+0] ; load closure unwrapper entry point
    leave ; unwind before jumping
    jmp rax ; tail call into closure
...
foo_unwrapper:
    ...
    mov [rbp-8], rdi ; store env_end arg in frame
    mov r12, [rbp-8] ; load operand
    mov rax, [r12-16] ; load remaining env field
    mov [rbp-16], rax ; store value
    mov rax, [r12-8] ; load done env field
    mov [rbp-24], rax ; store value
    mov rdi, r12 ; use pinned __env_end env_end pointer
...
_0_data:
    db "bar,", 0
_8:
    dq _8_data, 4, 0, 0 ; data, byte length, heap base, heap size
_8_data:
    db "foo,", 0
